...
class CustomTransformer(nn.Module):
    ...
    def __init__(self, d_model=512, d_embed=256, nhead=8, num_encoder_layers=6,
        ...
        encoder_layers = [ CustomTransformerEncoderLayer(d_model, d_embed, nhead, dim_feedforward, dropout,
                                                      layer_norm_eps, batch_first, norm_first) for i in range(num_encoder_layers) ]
        encoder_norm = LayerNorm(d_model, eps=layer_norm_eps)
        self.encoder = CustomTransformerEncoder(encoder_layers, num_encoder_layers, encoder_norm)

        decoder_layers = [ CustomTransformerDecoderLayer(d_model, d_embed, nhead, dim_feedforward, dropout,
                                                      layer_norm_eps, batch_first, norm_first) for i in range(num_decoder_layers) ]
        decoder_norm = LayerNorm(d_model, eps=layer_norm_eps)
        self.decoder = CustomTransformerDecoder(decoder_layers, num_decoder_layers, decoder_norm)
    ...
    def forward(self, src, tgt, src_spk, src_mask=None, tgt_mask=None,
        ...
        output = self.decoder(tgt, memory, src_spk, tgt_mask=tgt_mask, memory_mask=memory_mask,
                              tgt_key_padding_mask=tgt_key_padding_mask,
                              memory_key_padding_mask=memory_key_padding_mask)
        return output
