...
def train(network, device, train_loader, optimizer, scheduler, epoch,
    ...
    for batch_idx, _data in enumerate(train_loader):
        inputs, labels, ctc_labels, input_lengths, label_lengths, ctc_label_lengths,  _ = _data

        input_lengths = network.valid_input_lengths(input_lengths)
        
        inputs, labels, ctc_labels = inputs.to(device), labels.to(device), ctc_labels.to(device)
        input_lengths, label_lengths, ctc_label_lengths = torch.tensor(input_lengths).to(torch.int32), torch.tensor(label_lengths).to(torch.int32), torch.tensor(ctc_label_lengths).to(torch.int32)
        input_lengths.to(device), label_lengths.to(device), ctc_label_lengths.to(device)

        optimizer.zero_grad()
        loss = network(inputs, labels, ctc_labels, input_lengths, label_lengths, ctc_label_lengths)
        loss.backward()
    ...
    with torch.no_grad():
        for i, _data in enumerate(test_loader):
            ...
            for j in range(inputs.shape[0]):
                ...
                #pred,_ = network.beam_search(torch.unsqueeze(inputs[j],0))
...
def decode(network, device, test_loader, tokenizer, outpath, beam_search=False):
    ...
    with open(outpath, 'w') as f:
        with torch.no_grad():
            for i, _data in enumerate(test_loader):
                inputs, labels, _, input_lengths, label_lengths, _, keys = _data
