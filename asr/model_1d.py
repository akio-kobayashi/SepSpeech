...
class LearnableEncoder(nn.Module):
    ...
    def valid_lengths(self, lengths):
        lengths = [ int( (length + 2* 200 - (400-1)) /160 + 1) for length in lengths ]
        return lengths
...
class Transducer(nn.Module):
    def __init__(self, device, vocab_size, hidden_size=144,
        ...
        self.encoder = LearnableEncoder(chout=hidden_size)
        ...
        for n in range(num_layers):
            layers.append(
                ConformerBlock(
                    dim = hidden_size,
                    dim_head = hidden_size//self.num_heads,
        ...
        self.downsampler=CNTF1D(hidden_size, hidden_size, cntf_channels=hidden_size)
    ...
    def valid_input_lengths(self, lengths):
        lengths = self.encoder.valid_lengths(lengths)
        if self.downsampler is not None:
            lengths = self.downsampler.valid_lengths(lengths)
        return lengths
    ...
    def forward(self, xs, ys, xlen, ylen, return_loss=True):
        ...
        _, T, N, _ = out.shape
        #if torch.max(xlen) > T:
        xlen = torch.where(xlen > T, torch.tensor(T-1, dtype=torch.int32, device=xlen.device), xlen)
        if torch.max(xlen) < T:
            xlen += 1
        #if torch.max(ylen) > N:
        #ylen = torch.where(ylen > N, torch.tensor(N-1, dtype=torch.int32, device=ylen.device), ylen)
        #if torch.max(ylen) < N:
        #    ylen += 1
        try:
