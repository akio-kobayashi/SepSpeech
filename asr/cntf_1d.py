...
class CNTF1D(nn.Module):
    def __init__(self, in_channels, out_channels, depth=2, cntf_channels=104, kernel_size=3, repeat=3) -> None:
        super().__init__()
        self.kernel_size=kernel_size
        self.repeat=repeat
        self.cntf = nn.Sequential(
            #Res2NetBlock(1, cntf_channels),
            nn.Conv1d(in_channels, cntf_channels, kernel_size, stride=2, padding=kernel_size//2),
            ...
            nn.Conv1d(cntf_channels, 2*cntf_channels, kernel_size, stride=2, padding=kernel_size//2),
        ...
        self.final = nn.Conv1d(2*cntf_channels, out_channels, kernel_size, padding=kernel_size//2)
